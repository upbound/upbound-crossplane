...
- [ ] Created and merged a PR to the `main` branch of [upbound/upbound-crossplane][uxp] with the following changes:
...
- [ ] Cut [UXP][uxp] `vX.Y.Z-up.1` release from the `release-X.Y` branch by:
  ...
  - [ ] Verify that the tagged build version exists, on the [releases.upbound.io](https://releases.upbound.io/crossplane/) `build` channel, e.g. `build/release-X.Y/vX.Y.Z-up.K/...`
...
- [ ] Verify the produced helm chart available in the `build` channnel at `build/release-X.Y/vX.Y.Z-up.K/charts` by doing some sanity checks:
  - [ ] Installs on a cluster properly with `helm -n upbound-system upgrade --install upbound-crossplane <path-to-chart.tgz> --create-namespace`.
  ...
  - [ ] Upgrading from the latest supported version works, for example run:
    ...
    - upgrade to this new version as above: `helm -n upbound-system upgrade --install upbound-crossplane <path-to-chart.tgz> --create-namespace`
- [ ] Run the [Promote workflow][promote-uxp] from the `release-X.Y` branch, to promote `vX.Y.Z-up.1` to `stable`, [here][uxp-stable-channel] you should find `upbound-crossplane-X.Y.Z-up.1.tgz`. Verify everything is correctly working by running `up uxp install` against an empty Kubernetes cluster, e.g. `kind create cluster`, which should result in an healthy UXP installation with expected image versions.
...
<!-- Named Links -->
[ci-uxp]: https://github.com/upbound/upbound-crossplane/actions/workflows/ci.yml
[ci-xp-fork]: https://github.com/upbound/crossplane/actions/workflows/ci.yml
[promote-uxp]: https://github.com/upbound/upbound-crossplane/actions/workflows/promote.yml
[sync-xp-fork]: https://github.com/upbound/upbound-crossplane/blob/main/docs/CONTRIBUTING.md#crossplane-fork-sync
[tag-uxp]: https://github.com/upbound/upbound-crossplane/actions/workflows/tag.yml
...
[uxp-releases]: https://github.com/upbound/upbound-crossplane/releases
[uxp-schedule]: https://github.com/upbound/upbound-crossplane/blob/main/README.md#releases
[uxp-stable-channel]: https://charts.upbound.io/stable
[uxp-values]: https://github.com/upbound/upbound-crossplane/blob/main/cluster/charts/upbound-crossplane/values.yaml.tmpl
[uxp]: https://github.com/upbound/upbound-crossplane
